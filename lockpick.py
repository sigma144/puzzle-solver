...
class LockpickState:
    ...
    def __repr__(self) -> str:
        ...
        for e in self.edges:
            s += str(e[0]) + ' -> ' + ' '.join([l[0]+str(l[1])+l[2] for l in e[1]]) + ' -> ' + str(e[2]) + '\n'
        return s
    ...
    def open(self, i, si, aura, sign=1):
        ...
        if 'X' in num:
            ...
            if stack_limit and max(abs(imag), abs(real)) > stack_limit:
                return False
            ...
            if real != 0 or imag != 0:
                seq[si] = (aura, color, num + 'X' + str(real) + (imag_part or ''))
                self.terminate = True
                return True
        elif sign == -1:
            seq[si] = (aura, color, num + 'X' + ('1+1i' if self.iview else '2'))
            self.terminate = True
            return True
        seq.pop(si)
        if len(seq) == 0:
...
pattern = r'([!#@]*)((?:[A-WYZ]/)?(?:[a-hj-wyzA-WYZ$<>]|\[[^\[]*\]))(-?\d*i?(?:[\+-]\d*i)?X-?\d+(?:[\+-]\d*i)?|-?xi?|-?\+|=?-?\*?\d*i?(?:[\+-]\d*i)?)'
roomi = 0

def parse(level, target_moves=0):
    global roomi
    roomi = 1
...
def parse_deprecated(stock, edges, target_moves=None):
    ...
    for k in parse:
        if len(k[2]) == 0:
            ...
            real, imag = parse_complex(k[2])
            if real: state.stock[k[1]] = int(real)
            state.stock[k[1]+'i'] = int(imag[:-1])
        else:
...
class LockpickSolver(Solver):
    ...
    def check_state(self, state): #Place to add some extra logic
        ...
        if self.special == '1-B':
            ...
            if not state.edges[5][1] and state.stock.get('w', 0) == 0 and (not state.edges[4][1] or ('', 'W', '0') in state.edges[4][1]):
                return False
            ...
            if state.last_move in [7, 8] and prev.last_move not in [7, 8]:
                if len(prev.edges[4][1]) < 2 or prev.stock.get(prev.edges[4][1][0][0][-1].lower(), 0) > 0:
                    return False
            elif prev.last_move in [7, 8] and state.last_move not in [7, 8, 4]:
                if prev.stock.get(prev.edges[4][1][0][0][-1].lower(), 0) > 0:
                    return False
    ...
    def solve(self, state, debug=False, print_moves=True, verbose=False):
        ...
        limit = {p87: 2, p8A: 3, p9C: 2, p1010: 2, p10C: 2, p119: 1, p11A: 2}
        global stack_limit
        if state in limit:
...
p1B = parse('w5o2p3g4k2((PPcGG|WKcWW|WKcGP|GWcKO|WWcWG|OOcKO|GPcPW|GOcOW|).)|C8$', 10) #Uses special logic
...
p210 = parse('o(O0o|OoP2c|OoC2p2|Oc|Po4|O0(P2p2C2O0P0$|Oc2Po2|PCpO2c2', 12) #Bridge edge case (special logic)
...
p45 = parse('w(Kw!Orw|W4r|WoKo!CkR0O0!Oow|Oc2Cw3Ro|OcKoWo2Ow|OkCk!W0o|Wo!Co!Rc|!O2!W2$', 18)
...
p53 = parse('m24(M4M2|N3MM3)(M3M2M3|MM8)(M6n10|M3M2(M3M3N3|M3M0N0|MM4M3M)(M2N2N4M2|M2M4M2M)M24M0$', 10) #Uses special logic
...
p5B = parse('m2(B3g8w4U3m2U2|W4(G2u9k12Mg2Gx|u=5(UU0gK4|RxKK6(G4U2w5|C3@U0K4m8g=2(W2G3M2u|K6G0W0u|K3W5W0@K0PG0W(G2@G0u|U2W2u2|U3U0$', 24) #Takes a while (1.9 min), uses special logic
...
p610 = parse('m(U25$|(PxbUu4#B3|Uxu4U2bC-x|p-5P-xu5B0c-C3n3U|c9C3b#P0u-#P0u-4u-M-3u4N0|P-4p4C-xu-U3u3b=0c-U-3)u-6', 19) #Uses special logic
...
p73 = parse('o5c7C/OxO/Cx(C/Ox(c-|C/Oxc)|O0C0$|O/Cx(o-|O/Cxo)', 8)
...
p7D = parse('!W12$|C2C0O2w2|KC2Ow2|KR2w2|K12|o10k10c10(NN0K0O0C2w6|NC3KO6|OKk2|C4n13|N/K4rN8w6', 16) #Takes a bit (0.7 min)
p7E = parse('c-|o-|p-|cccccccc|oooooooo|pppppppp|>(W/Cx|W/C-x)(W/OxW/Ox|W/O-xW/O-x)(W/PxW/PxW/Px|W/P-xW/P-xW/P-x)W17W0' + 
    '(W/Px|W/P-x)(W/CxW/Cx|W/C-xW/C-x)(W/OxW/OxW/Ox|W/O-xW/O-xW/O-x)W2W0(W/Ox|W/O-x)(W/PxW/Px|W/P-xW/P-x)(W/CxW/CxW/Cx|W/C-xW/C-xW/C-x)W17W0$', 18) #Uses special logic
...
p85 = parse('w16p-8(P-2W4M/P-8M-2W/P-4|W2P-4M8W6M0W|M/P-2W5P-2M/W2M0P-1P-1|W0|M/P0M/W0($)', 11) #Special logic for bridge
p86 = parse('b(BU4M0M/B0M/W0M/C0$|BM0mw12C12bu|BM0C4m2bCc5u|BM0W4m-2bWw5u|BM0m-1c12W12bu', 13)
p87 = parse('m-1m*c-4(P/C-1|P/C-4|K/C-1|K/C-4|C-x>m-*m=1M/KxC/PxM0C-84C0C/KxC/PxC-25C0$', 19) #Special logic to limit stacking
...
p93 = parse('(wWxw4Wxw|o2Oxo2Oxo3|pPxp3Pxp2|c3Cxc2Cxc)(wWxwWxw=1|o2Oxo2Oxo=1|p3Pxp3Pxp=1|c3Cxc2Cxc=1)B/[WxOxPxCx]B-32$', 16) #Timeout
...
p9E = parse('#U/N0$|[KU0]n90n*[BU0]b-1|k16[KxKx]|b-2[BxBx]|m4[MxMxU0]|b-*[N-xN-xU0]|([B-xB-x][M2U0][K-xK-x]M-4W/[M0U0]|B2M6B2K17W/[M0U0])(K-9|b-*)|(B2N-8[KU0][K3U0]W/[K0U0]|B-2[NxNxNxNx][K-1U0]K4W/[K0U0])(W70|b*)') #Timeout
...
p103 = parse('w7o6B/[W0O0P0C0]$|C0O0P/Z16|O4O0Z|c6W/P0([ZZZZZZ]W4|OO0Z|p16P0|WP0|OC/O0)', 19) #Bridge special case
...
p106 = parse('w50u-7(U-2p*o*w-50|U-2o-7O-1n6|U-2o-4O-6O0P5|U-2p5P6P0O-4|U-2M-1m*n*z50|N0(W0u-4m-1|Z50$', 11) #Curse special case
...
p1010 = parse('u4m-1m3M/[OxGxBxRxU0]$|U[Z-xZ-x]|U[ZxZxZx]|UZ/Mx|UZ/U|UZ/[OOOO]|U[NxN-x]|UO/[]R/[]G/[]B/[]|UM/[]N/[]M/[]N/[]', 30) #Takes a long time (7 min)
p10A = parse('Z|Z2|Z4|Z8|Z16|Z32|Z64|B/[M-1U]|uK[M-1U]|C/[M-1U]|m-3nn-2|u-1k43b92c5W/[B0K0C0U0]$') #Curse special case, timeout
p10B = parse('(>m-1>|>k-1>|>c-1>)w*(m=0Z/W5[Z-5U0]$|n-|Ck*n=0Cw-*|Kc*n=0Kw-*|Mn*k4c4w5|N0|n2', 11) #Brown bridge special case
p10C = parse('c50b-90m-1m*(z-b-z-|[C10Zx]|[B-20Z-x]|O/[C0B0U0]$|n10n10n10n10n10n10n10n10n10n10n10n10n10n10n10n10n10n10n10n10', 25) #Takes a very long time (15 min)
...
p119 = parse('n-n-n-|n55+28iU/N0$|m1-2i|KN-8i|KN-32|K-4iN-16N2|K4iK-8i', 16) #Takes a bit (0.5 min)
...
p11A = parse('m-1m*o-6(O-1|W/O-x|R/O-x|G/O-x|B/O-x|>o=0-1ir-K/[WxR-x]K=K/[GxR-x]K=K/[BxR-x]K=g-K/[WxG-x]K=K/[BxG-x]K=b-K/[WxB-x]K=r-g-b-' +
    'k30K/[O-1iWx]C/[U0K+]K+K0k30K/[O-1iRx]C/[U0K+]K+K0k30K/[O-1iGx]C/[U0K+]K+K0m-*k30K/[O-1iBx]C/[U0K+]K+K0M-1C-15iC0$') #Timeout
pEx = parse('ow(OpBwPkCb|Wo(BWo|PkCbBB$|KcBwKc|OpKcWo|PkOpKcCb', 7)
...
finale = parse('m2wp8((C2WPg-WOx|KWO-xK12c-12)b-u|W0C-2M0o=1(w(Wo=-1o=-1!C!Cg-5u|N-3O-xKWC8u)|mWO0O0M0(K4C2OxBP-6W!O0u|c4w2((#C2WP-1wG4B0|C2WPk17K12C3)ru|m2o=1W0C0C0O0O0O0M0c6b(p-|P6w2#O0u|m3K12BP-1w2o=-1(' +
    'P0B@O0u|k31O0O0M0m(RN-3B0g-5|(K4w2P-1@C@C|BG12Pn-99C)O0b2u|WWC4o=1M0m2(C0WOxKW0|PPC-12O0W0P-1P-1)(r-RGxWc-u|R0W0M0(o=1!W0G-xC0C0u|m5U10M0O0K0R0G0B0n92N0$') #Put in for kicks and giggles, I highly doubt it will solve this!
...
#test(full=0, print_moves=0) #Time: ~95 sec
#practice()

LockpickSolver().solve(p4B, verbose=0, debug=0)